...
public class Environment : MonoBehaviour {
    ...
    public static LivingEntity SenseFood (Coord coord, Animal self, System.Func<LivingEntity, LivingEntity, int> foodPreference) {
        // initiate an empty list that will be filled with food sources that are of the type LivingEntity, this means it can be animal or plant
        var foodSources = new List<LivingEntity> ();

        // initiate a list of prey by accessing the prey for the animal called self, by species using a dictionary that contains prey for that animal self
        List<Species> prey = preyBySpecies[self.species];
        // loop through different types of prey
        for (int i = 0; i < prey.Count; i++) {

            Map speciesMap = speciesMaps[prey[i]];

            // add all the different food sources by range according to their cordinates and based on the view distance
        ...
        // Return first VISISBLE food source (maybe make it so that only food that is visible is added to optimize)
