...
public class Environment : MonoBehaviour {
    ...
    [Range(0, 1)]
    public float maxUpdatePlantPop;
    public float updatePlantProb;
    ...
    void Update() {
        ...
        {
            if (pop.prefab is Plant) {
                for (int i = 0; i < maxUpdatePlantPop; i++)
                {
                    if (prng.NextDouble() < updatePlantProb)
                    {
                        ...
                        // Spawn
                        // MeshRenderer plant = Instantiate(pop.prefab);
    ...
    void SpawnInitialPopulations () {
        ...
        // Loop through each species or population in initialPopulations
        foreach (var pop in initialPopulations) {
            // Loop n times to spawn a living entitiy of that type
            for (int i = 0; i < pop.count; i++) {
                if (spawnCoords.Count == 0) {
                    Debug.Log ("Ran out of empty tiles to spawn initial population");
                    break;
                }
                // From the list of spawnCoords randomly choose one from the list
                int spawnCoordIndex = spawnPrng.Next (0, spawnCoords.Count);
                Coord coord = spawnCoords[spawnCoordIndex];
                // Remove that coordinate from the list of avaliable spots
                spawnCoords.RemoveAt (spawnCoordIndex);
                ...
                // Add them to the map
                // Think of it like 2D, an animation then the actual meat of the program below
                speciesMaps[entity.species].Add (entity, coord);
            }
        }
    }
